...
export const serializeAuditDates = <T>(
  input: WithAuditDates<T>,
): SerializedAuditDates<T> => {
  const serialized = { ...input } as SerializedAuditDates<T>;

  if (input.createdAt instanceof Date)
    serialized.createdAt = input.createdAt.toISOString();
  if (input.updatedAt instanceof Date)
    serialized.updatedAt = input.updatedAt.toISOString();
